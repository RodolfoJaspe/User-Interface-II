...
Fixed layouts are designed for a specific size and do not adjust when the viewport size changes. Fluid layouts replace px sized containers with % allowing them tho change size according to the size of the viewport. Adaptive layout introduce media query breakpoints at which new design rules come in to adjust to a different screen size. Responsive layouts include all of the above plus make all the size units scalable so that the site fully adapts to all screen sizes and provides accessibility.
...
When switching to smaller sized viewports font-sizes can be increased so that it remains readable, this is especially important for those with visual disabilities. There are millions of people with disabilities that would prevent them from using websites if they all had fixed layouts. Through responsive design you can make sure everyone is included.
